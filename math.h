 ...
 */
static __inline__ Vec3 periodic(Vec3 period, Vec3 v)
{
	Vec3 res;
	Vec3 hp = scale(period, 1 / 2.0); /* Half Period */
	res.x = hp.x + floor((v.x - hp.x) / period.x) * period.x;
...
static __inline__ double _closePeriodic(double period, double val)
{
	...
	assert(-period/2.0 <= val  &&  val < period/2.0);
 ...
 * PostConditions:
 *     -period.x/2.0 <= res.x   &&   res.x < period.x/2.0
 *     -period.y/2.0 <= res.y   &&   res.y < period.y/2.0
 *     -period.z/2.0 <= res.z   &&   res.z < period.z/2.0
 */
static __inline__ Vec3 closePeriodic(Vec3 period, Vec3 v)
{
	Vec3 res;
	res.x = _closePeriodic(period.x, v.x);
	res.y = _closePeriodic(period.y, v.y);
	res.z = _closePeriodic(period.z, v.z);
	return res;
}
...
{
	assert(-1.5 * period <= val  &&  val < 1.5 * period);
 ...
 * PreConditions:
 *     -1.5 * period.x <= v.x   &&   v.x < 1.5 * period.x
 *     -1.5 * period.y <= v.y   &&   v.y < 1.5 * period.y
 *     -1.5 * period.z <= v.z   &&   v.z < 1.5 * period.z
 * PostConditions:
 *     -period.x/2.0 <= res.x   &&   res.x < period.x/2.0
 *     -period.y/2.0 <= res.y   &&   res.y < period.y/2.0
 *     -period.z/2.0 <= res.z   &&   res.z < period.z/2.0
 */
static __inline__ Vec3 fastPeriodic(Vec3 period, Vec3 v)
{
	Vec3 res;
	res.x = _fastPeriodic(period.x, v.x);
	res.y = _fastPeriodic(period.y, v.y);
	res.z = _fastPeriodic(period.z, v.z);
	return res;
}
